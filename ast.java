...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() throws EmptySymTableException {
        ...
        // check main
        try {
            TSym sym = symTab.lookupGlobal("main");
...
class DeclListNode extends ASTnode {
     ...
     * decls in the list.
     */
    public int nameAnalysis(SymTable symTab, SymTable globalTab) {
        int size = 0;
        int offset = 4;
        for (DeclNode node : myDecls) {
            if (node instanceof VarDeclNode) {
                //if global
                if (globalTab.getSize() == 1) {
                    //global
                    ((VarDeclNode)node).nameAnalysis(symTab, globalTab, true, 0);
                }
                else {
                    ((VarDeclNode)node).nameAnalysis(symTab, globalTab, false, offset);
                    offset += 4;
                    size += 4;
                } 
            } 
    ...
    public List<Type> nameAnalysis(SymTable symTab) {
        ...
        for (FormalDeclNode node : myFormals) {
            ...
            if (sym != null) {
                ...
                // each sym of param to its offset
        ...
        this.size = -currOffset;
        return typeList;
    }

    public void codeGen() {
        int offset = 8;
        for(FormalDeclNode formals : myFormals){
            offset += 4;
        }
        Codegen.generate("add", Codegen.FP, Codegen.SP, offset);
...
class FnBodyNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTab) {
        localSize = myDeclList.nameAnalysis(symTab);
...
class VarDeclNode extends DeclNode {
    ...
    public TSym nameAnalysis(SymTable symTab) {
        return nameAnalysis(symTab, symTab, false, -1);
    }

    public TSym nameAnalysis(SymTable symTab, SymTable globalTab, boolean isGlobal, int localOffset) {
...
class FnDeclNode extends DeclNode {
    ...
    public TSym nameAnalysis(SymTable symTab) {
        ...
        if (sym != null) {
            sym.setSizeLocals(myBody.getLocalSize());
            sym.setSizeParams(myFormalsList.getSize());
        }
    ...
    public void codeGen() {
        ...
        //exit
        // Need to add this exit label based on in example
        String exitLabel = "_"+myId.name()+"_Exit";
        myBody.codeGen(exitLabel);

        Codegen.genLabel(exitLabel);
        Codegen.p.println("\t\t\t#FUNCTION EXIT");
        Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, ((FnSym)myId.sym()).getSizeParams() * (-1), "load return address"); 
        Codegen.generate("move", Codegen.T0, Codegen.FP);

        Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP, (((FnSym)myId.sym()).getSizeParams() * (-1)) - 4, "restore FP"); 
...
abstract class StmtNode extends ASTnode {
    ...
    public abstract void codeGen(String exitLabel); 
}
...
class PostDecStmtNode extends StmtNode {
    ...
    public void codeGen(String exitLabel) {
		...
		Codegen.generateIndexed("sw", "$t0", "$fp", sym.getOffset());
	    }
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void codeGen(String exitLabel) {
        ...
        String trueEndLabel = Codegen.nextLabel();
        String falseEndLabel = Codegen.nextLabel();
        // the eval is on the stack
        //pop
        Codegen.genPop(Codegen.T1);
        Codegen.generate("beq", Codegen.T1, Codegen.FALSE, trueEndLabel);
        myThenStmtList.codeGen(exitLabel);
        Codegen.generate("b", falseEndLabel);
        Codegen.genLabel(trueEndLabel);
        myElseStmtList.codeGen(exitLabel);
        Codegen.genLabel(falseEndLabel);
    }
...
class WhileStmtNode extends StmtNode {
    ...
    public void codeGen(String exitLabel) {
        ...
        Codegen.genPop(Codegen.T1);
        Codegen.generate("beq", Codegen.T1, Codegen.FALSE, endLabel);
        myStmtList.codeGen(exitLabel);
...
class IntLitNode extends ExpNode {
    ...
    public void codeGen() {
        Codegen.generate("li", Codegen.T0, String.valueOf(this.myIntVal));
        Codegen.genPush(Codegen.T0); 
    }
...
class AssignNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        //one pop for the address
        
        Codegen.genPop(Codegen.T1);
        Codegen.genPop(Codegen.T0);
        
        // Putting value into t0
        // Codegen.generateIndexed("sw", Codegen.T0, Codegen.SP, 4);
        // putting value into the address stored in t1
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.generate("beq", Codegen.T0, "0", branchLabel);
...
class OrNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        String branchLabel = Codegen.nextLabel();
        Codegen.generate("beq", Codegen.T0, "1", branchLabel);
